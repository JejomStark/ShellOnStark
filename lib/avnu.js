...
async function promptOrderSetup() {
    ...
    while (true) {
        try {
            ...
            if (orderTypeAnswer.orderType.toLowerCase() === 'exit') {
                return;
            }
            const orderType = orderTypeAnswer.orderType.split(' - ')[0];
            // Common questions for all order types
            const orderQuestions = [
                ...
                },
                {
                    type: 'input',
                    name: 'price',
                    message: `Set the ${orderType} price`,
                    validate: value => {
                        if (isNaN(value) || Number(value) <= 0) {
            ...
            const orderAnswers = await inquirer.prompt(orderQuestions);
            

            const orderInfo = {
                type: orderType.replace(' ', '_').toLowerCase(),
                from: orderAnswers.from.toUpperCase(),
                to: orderAnswers.to.toUpperCase(),
                amount: orderAnswers.amount,
                price: orderAnswers.price
            }
            const confirmation = await inquirer.prompt([
                {
                    type: 'confirm',
